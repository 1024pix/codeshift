...
function _updateExistingAnswer(existingAnswer, newAnswer, h) {
  return solutionRepository
    ...
    .then((updatedAnswer) => {
      return h.response(answerSerializer.serializeFromBookshelfAnswer(updatedAnswer)).code(200);
    })
    .catch((err) => {
      logger.error(err);
      h.response(Boom.badImplementation(err));
    });
}
...
module.exports = {
  ...
  get(request, h) {
    ...
    new BookshelfAnswer({ id: request.params.id })
      .fetch()
      .then((answer) => h.response(answerSerializer.serializeFromBookshelfAnswer(answer)))
        ...
        return isAssessmentSmartPlacement(existingAnswer.get('assessmentId'))
          .then((assessmentIsSmartPlacement) => {
            ...
            if (assessmentIsSmartPlacement) {
              ...
              return _updateExistingAnswer(existingAnswer, updatedAnswer, h);
            }
          })
  ...
  findByChallengeAndAssessment(request, h) {
    return answerRepository
      ...
      .then((answer) => {
        return h.response(answerSerializer.serializeFromBookshelfAnswer(answer)).code(200);
      })
      .catch((err) => {
        logger.error(err);
