...
describe('Unit | Controller | assessment-results', () => {
  ...
  describe('#evaluate', () => {
    ...
    it('should evaluate the assessment', () => {
      // when
      assessmentResultController.evaluate(request, hStub);
    ...
    it('should return 404 when the assessment is not found', () => {
      ...
      // when
      const promise = assessmentResultController.evaluate(request, hStub);

      // then
      return promise.then(() => {
        expect(Boom.notFound).to.have.been.calledWith(notFoundAssessmentError);
        expect(hStub.response).to.have.been.calledWith({ message: 'NotFoundError' });
      });
    });
    ...
    context('when the assessment is already evaluated', () => {
      ...
      it('should do nothing', () => {
        ...
        // then
        return promise.then(() => {
          expect(hStub.response).to.have.been.called;
          expect(hStub.response).to.have.been.calledWith(jsonApiError);
        });
      });
    });
    ...
    context('when the database is fail', () => {
      ...
      it('should reply with an internal error', () => {
        ...
        return promise.then(() => {
          ...
          expect(hStub.response).to.have.been.calledWith(Boom.badImplementation());
        });
      });
    });
  });
